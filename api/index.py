...
def run_command(cmd):
    try:
        ...
        # Handle cases where system commands aren't available in serverless environment
        result = f"Command not available in serverless environment: {cmd}"
    return result
...
def log_snapshot():
    ...
    try:
        ...
        flash(f"Error saving snapshot: {str(e)}")
...
def alerts():
    ...
    if not alert_output.strip() or "Command not available" in alert_output:
        alert_output = 'No high resource usage detected or monitoring unavailable in serverless environment.'
